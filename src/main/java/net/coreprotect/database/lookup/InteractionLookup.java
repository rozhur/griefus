    ...
    public static String performLookup(String command, Statement statement, Block block, CommandSender commandSender, int offset, int page, int limit) {
        ...
        try {
            ...
            while (results.next()) {
                ...
                resultBuilder.append('\n').append(Phrase.build(!rb ? Phrase.GENERIC_LOOKUP_FORMAT : Phrase.GENERIC_LOOKUP_FORMAT_RB, timeAgo, resultUser, dselector, target, ""));
            ...
            if (found) {
                if (count > limit) {
                    ...
                    String pageInfo = "\n" + ChatUtils.getSimplePageNavigation(command, page, totalPages);
